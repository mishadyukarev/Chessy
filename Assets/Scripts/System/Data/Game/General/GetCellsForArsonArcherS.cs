    ...
    public struct GetCellsForArsonArcherS : IEcsRunSystem
    {
        public void Run()
        {
            foreach (var idx_0 in Entities.CellEs.Idxs)
            {
                ...
                ref var unit_from = ref Entities.CellEs.UnitEs.Else(idx_0).UnitC;
                ref var ownUnit_from = ref Entities.CellEs.UnitEs.Else(idx_0).OwnerC;
                ref var stun_from = ref Entities.CellEs.UnitEs.Stun(idx_0).ForExitStun;
                ...
                if (!stun_from.Have)
                {
                    if (unit_from.Is(UnitTypes.Archer))
                    {
                        ...
                        {
                            ref var fire_1 = ref Entities.CellEs.FireEs.Fire(idx_1).Fire;
