...
{   
    public readonly struct EntityLeftCityUIPool
    {
        ...
        public EntityLeftCityUIPool(in WorldEcs gameW, in Transform leftZone)
        {
            var buildZone = leftZone.transform.Find("BuildingZone");

            _ents.Add(LeftCityEntType.Melt, gameW.NewEntity()
                .Add(new ButtonVC(buildZone.Find("MeltOreButton").GetComponent<Button>())));

            _buyEnts[ResTypes.Food] = gameW.NewEntity().Add(new ButtonVC(buildZone.Find("UpgradeFarm_Button").GetComponent<Button>()));
            _buyEnts[ResTypes.Wood] = gameW.NewEntity().Add(new ButtonVC(buildZone.Find("UpgradeWoodcutter_Button").GetComponent<Button>()));
        }
