...
namespace Game.Game
{
    public readonly struct EntityLeftEnvUIPool
    {
        ...
        public EntityLeftEnvUIPool(in WorldEcs gameW, in Transform leftZone)
        {
            var envZone = leftZone.Find("EnvironmentZone");

            _ents[LeftEnvEntType.Info] = gameW.NewEntity().Add(new ButtonVC(envZone.Find("EnvironmentInfoButton").GetComponent<Button>()));

            _envs[ResTypes.Food] = gameW.NewEntity().Add(new TextUIC(envZone.Find("FertilizerResources_TextMP").GetComponent<TextMeshProUGUI>()));
            _envs[ResTypes.Wood] = gameW.NewEntity().Add(new TextUIC(envZone.Find("ForestResources_TextMP").GetComponent<TextMeshProUGUI>()));
            _envs[ResTypes.Ore] = gameW.NewEntity().Add(new TextUIC(envZone.Find("OreResources_TextMP").GetComponent<TextMeshProUGUI>()));
        }
