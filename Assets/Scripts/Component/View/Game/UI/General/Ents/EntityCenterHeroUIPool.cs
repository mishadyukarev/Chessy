...
namespace Game.Game
{
    public readonly struct EntityCenterHeroUIPool
    {
        ...
        public EntityCenterHeroUIPool(in EcsWorld gameW, in Transform centerZone)
        {
            var parent = centerZone.transform.Find("HeroesZone");

            _ents[UnitTypes.Elfemale] = gameW.NewEntity().Add(new ButtonUIC(parent.Find("Elffemale_But").GetComponent<Button>()));
            _ents[UnitTypes.None] = gameW.NewEntity().Add(new ButtonUIC(parent.Find("Premium_Button").GetComponent<Button>()));
        }
    }
}