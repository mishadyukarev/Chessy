    ...
    public sealed class SystemsViewUIGame : IEcsRunSystem
    {
        ...
        public void Run()
        {
            ...
            if (_e.NeedUpdateView || _timer > 0.5f)
            {
                ...
                _eUIGame.CenterEs.MistakeE.Sync(_timer, _e);
                MotionUIS.Sync(_timer, _eUIGame, _e);
                new UpSunsUIS(_eUIGame, _e).Run();


                var nextPlayerT = _e.CurPlayerITC.PlayerT.NextPlayer();
                var haveElfemaleEnemy = _e.PlayerInfoE(nextPlayerT).MyHeroTC.Is(UnitTypes.Elfemale);
                var haveSnowyEnemy = _e.PlayerInfoE(nextPlayerT).MyHeroTC.Is(UnitTypes.Snowy);
