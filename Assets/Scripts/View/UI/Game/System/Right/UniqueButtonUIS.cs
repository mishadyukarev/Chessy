...
namespace Chessy.Game.View.UI.System
{
    sealed class UniqueButtonUIS : SystemUIAbstract
    {
        ...
        internal UniqueButtonUIS(in ButtonTypes buttonT, in UniqueButtonUIE uniqueButtonUIE, in Resources res, in Chessy.Game.Model.Entity.EntitiesModelGame ents) : base(ents)
        {
            _buttonT = buttonT;
        ...
        internal override void Sync()
        {
            ...
            if (e.UnitPlayerT(e.SelectedCell) == e.CurPlayerIT && ability_cur != AbilityTypes.None)
            {
                ...
                {
                    if (!e.LessonTC.HaveLesson || e.LessonTC.LessonT >= Enum.LessonTypes.Build3Farms)
                    {
                        needActive = true;
                    }
                }
                ...
                {
                    if (!e.LessonTC.HaveLesson || e.LessonTC.LessonT >= Enum.LessonTypes.PawnFireAdultForest)
                    {
                        needActive = true;
                    }
                }
