            ...
            for (byte idx_0 = 0; idx_0 < e.LengthCells; idx_0++)
            {
                ...
                switch (curPlayer)
                {
                    case PlayerTypes.First:
                        eV.CellEs(idx_0).TrailCellVC(DirectTypes.Up).ParentTransform.localEulerAngles = new Vector3(0, 0, 0);
                        break;

                    case PlayerTypes.Second:
                        eV.CellEs(idx_0).TrailCellVC(DirectTypes.Up).ParentTransform.localEulerAngles = new Vector3(0, 0, 180);
                        break;
