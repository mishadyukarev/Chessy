...
namespace Chessy.Game
{
    sealed class SyncKingVS
    {
        ...
        internal void Sync(in byte cell)
        {
            ...
            {
                if (_e.UnitEs(cell).ForPlayer(_e.CurPlayerITC.PlayerT).IsVisible)
                {
                    var isSelected = cell == _e.CellsC.Selected;
                    var isVisForNext = _e.UnitEs(cell).ForPlayer(_e.CurPlayerITC.PlayerT.NextPlayer()).IsVisible;

                    var unitT = _e.UnitTC(cell).UnitT;
                    ...
                    if (unitT == UnitTypes.Pawn)
                    {
                        ...
                        {
                            sr = _eV.UnitEs(cell).MainBowCrossbowE(isSelected, _e.UnitMainTWLevelTC(cell).LevelT, _e.UnitIsRightArcherC(cell).IsRight);
                        }
                        else
                        {
                            sr = _eV.UnitEs(cell).MainToolWeaponE(isSelected, _e.UnitMainTWLevelTC(cell).LevelT, _e.UnitMainTWTC(cell).ToolWeaponT);
                        }
                        ...
                        {
                            var twT = _e.UnitExtraTWTC(cell).ToolWeaponT;
                            var levT = _e.UnitExtraLevelTC(cell).LevelT;
                    ...
                    {
                        sr = _eV.UnitE(cell, isSelected, _e.UnitLevelTC(cell).LevelT, unitT);
                    }
