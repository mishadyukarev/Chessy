...
namespace Assets.Scripts
{
    ...
    {
        private static CellUnitsDataContainerEnts _cellUnitEntsContainer;



        internal CellUnitsDataWorker(CellUnitsDataContainerEnts cellUnitEntsContainer)
        {
        ...
        internal static void SetConditionType(ConditionUnitTypes protectRelaxType, int[] xy) => _cellUnitEntsContainer.CellUnitEnt_ProtectRelaxCom(xy).ProtectRelaxType = protectRelaxType;

        internal static void ResetConditionType(int[] xy) => SetConditionType(ConditionUnitTypes.None, xy);
        internal static bool IsConditionType(ConditionUnitTypes protectRelaxType, int[] xy) => _cellUnitEntsContainer.CellUnitEnt_ProtectRelaxCom(xy).ProtectRelaxType == protectRelaxType;
            ...
            else
            {
                for (DirectTypes directType1 = default; directType1 <= DirectTypes.LeftDown; directType1++)
                {
                    ...
                    if (CellViewWorker.IsActiveSelfParentCell(xy1))
                    {
                        ...
                        if (CellUnitsDataWorker.IsVisibleUnit(PhotonNetwork.IsMasterClient, xy2))
                        {
                            if (UnitType(xy) == UnitTypes.Rook || UnitType(xy) == UnitTypes.RookCrossbow)
                            {
