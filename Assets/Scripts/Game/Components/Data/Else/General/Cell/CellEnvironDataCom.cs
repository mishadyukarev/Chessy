...
namespace Scripts.Game
{
    internal struct CellEnvironDataCom
    {
        ...
        internal int AmountResources(EnvirTypes environmentType) => _amountResourcesOnCell[environmentType];
        internal void SetAmountResources(EnvirTypes environmentType, int value) => _amountResourcesOnCell[environmentType] = value;

        internal void AddAmountRes(EnvirTypes environmentType, int adding = 1) => SetAmountResources(environmentType, AmountResources(environmentType) + adding);
        internal void TakeAmountResources(EnvirTypes environmentType, int taking = 1) => SetAmountResources(environmentType, AmountResources(environmentType) - taking);

        internal bool HaveResources(EnvirTypes environmentType) => AmountResources(environmentType) > 0;
        internal bool HaveMaxRes(EnvirTypes envirType) => AmountResources(envirType) >= MaxAmountRes(envirType);

        internal int MaxAmountRes(EnvirTypes environmentTypes)
        {
        ...
        internal void SetNewEnvir(EnvirTypes environmentType)
        {
            ...
            switch (environmentType)
            {
                ...
                case EnvirTypes.Fertilizer:
                    randAmountResour = UnityEngine.Random.Range(MinAmountResources(environmentType), MaxAmountRes(environmentType) + 1);
                    break;
