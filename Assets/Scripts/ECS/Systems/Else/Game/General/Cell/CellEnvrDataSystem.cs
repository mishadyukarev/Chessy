    ...
    internal sealed class CellEnvrDataSystem : IEcsInitSystem, IEcsRunSystem
    {
            ...
            for (int x = 0; x < CELL_COUNT_X; x++)
                for (int y = 0; y < CELL_COUNT_Y; y++)
                {
                    _cellFertilizerEnts[x, y] = _gameWorld.NewEntity()
                        .Replace(new EnvironmentResourcesComponent())
        ...
        internal static int GetAmountResources(EnvironmentTypes environmentType, int[] xy)
        {
            switch (environmentType)
            {
                ...
                case EnvironmentTypes.Fertilizer:
                    return _cellFertilizerEnts[xy[X], xy[Y]].Get<EnvironmentResourcesComponent>().AmountEnvrRes;

                case EnvironmentTypes.YoungForest:
                    throw new Exception();

                case EnvironmentTypes.AdultForest:
                    return _cellAdultForestEnts[xy[X], xy[Y]].Get<EnvironmentResourcesComponent>().AmountEnvrRes;

                case EnvironmentTypes.Hill:
                    return _cellHillEnts[xy[X], xy[Y]].Get<EnvironmentResourcesComponent>().AmountEnvrRes;
        ...
        internal static void SetAmountResources(EnvironmentTypes environmentType, int value, int[] xy)
        {
            switch (environmentType)
            {
                ...
                case EnvironmentTypes.Fertilizer:
                    _cellFertilizerEnts[xy[X], xy[Y]].Get<EnvironmentResourcesComponent>().AmountEnvrRes = value;
                    break;
                ...
                case EnvironmentTypes.AdultForest:
                    _cellAdultForestEnts[xy[X], xy[Y]].Get<EnvironmentResourcesComponent>().AmountEnvrRes = value;
                    break;

                case EnvironmentTypes.Hill:
                    _cellHillEnts[xy[X], xy[Y]].Get<EnvironmentResourcesComponent>().AmountEnvrRes = value;
                    break;
