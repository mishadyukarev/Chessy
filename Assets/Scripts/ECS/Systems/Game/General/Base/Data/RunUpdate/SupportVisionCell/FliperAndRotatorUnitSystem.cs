            ...
            for (int x = 0; x < CellViewWorker.Xamount; x++)
                for (int y = 0; y < CellViewWorker.Yamount; y++)
                {
                    ...
                    if (CellUnitsDataWorker.HaveAnyUnit(xy))
                    {
                        ...
                        var standartX = CellViewWorker.GetEulerAngle(XyzTypes.X, xy);
                        var standartY = CellViewWorker.GetEulerAngle(XyzTypes.Y, xy);
                        var standartZ = CellViewWorker.GetEulerAngle(XyzTypes.Z, xy);
