...
internal sealed partial class EntitiesGeneralManager : EntitiesManager
{
    ...
    internal EntitiesGeneralManager(EcsWorld gameWorld)
    {


        #region Cells

        _cellEnts = new EcsEntity[startValues.CELL_COUNT_X, startValues.CELL_COUNT_Y];
        _cellUnitEnts = new EcsEntity[startValues.CELL_COUNT_X, startValues.CELL_COUNT_Y];
        _cellBuildingEnts = new EcsEntity[startValues.CELL_COUNT_X, startValues.CELL_COUNT_Y];
        _cellEnvironmentEnts = new EcsEntity[startValues.CELL_COUNT_X, startValues.CELL_COUNT_Y];
        _cellSupportVisionEnts = new EcsEntity[startValues.CELL_COUNT_X, startValues.CELL_COUNT_Y];
        _cellEffectEnts = new EcsEntity[startValues.CELL_COUNT_X, startValues.CELL_COUNT_Y];
        ...
        _buildingsEnt = gameWorld.NewEntity()
            .Replace(new BuildingsComponent(startValues))
    ...
    internal void SpawnAndFillEntities()
    {
        ...
        BackGroundGO = GameObject.Instantiate(Instance.ResourcesLoad.PrefabConfig.BackGroundCollider2D,
        ...
        MistakeAudioSource.clip = Instance.ResourcesLoad.SoundConfig.MistakeAudioClip;
        ...
        AttackAudioSource.clip = Instance.ResourcesLoad.SoundConfig.AttackAudioClip;
        ...
        if (Instance.IsMasterClient)
        {
            ...
            UnitInventorEnt_UnitInventorCom.SetAmountUnits(UnitTypes.King, true, startValues.AMOUNT_KING_MASTER);
            UnitInventorEnt_UnitInventorCom.SetAmountUnits(UnitTypes.King, false, startValues.AMOUNT_KING_OTHER);

            UnitInventorEnt_UnitInventorCom.SetAmountUnits(UnitTypes.Pawn, true, startValues.AMOUNT_PAWN_MASTER);
            UnitInventorEnt_UnitInventorCom.SetAmountUnits(UnitTypes.Pawn, false, startValues.AMOUNT_PAWN_OTHER);

            UnitInventorEnt_UnitInventorCom.SetAmountUnits(UnitTypes.Rook, true, startValues.AMOUNT_ROOK_MASTER);
            UnitInventorEnt_UnitInventorCom.SetAmountUnits(UnitTypes.Rook, false, startValues.AMOUNT_ROOK_OTHER);

            UnitInventorEnt_UnitInventorCom.SetAmountUnits(UnitTypes.Bishop, true, startValues.AMOUNT_BISHOP_MASTER);
            UnitInventorEnt_UnitInventorCom.SetAmountUnits(UnitTypes.Bishop, false, startValues.AMOUNT_BISHOP_OTHER);
            ...
            EconomyEnt_EconomyCom.SetFood(true, startValues.AMOUNT_FOOD_MASTER);
            EconomyEnt_EconomyCom.SetWood(true, startValues.AMOUNT_WOOD_MASTER);
            EconomyEnt_EconomyCom.SetOre(true, startValues.AMOUNT_ORE_MASTER);
            EconomyEnt_EconomyCom.SetIron(true, startValues.AMOUNT_IRON_MASTER);
            EconomyEnt_EconomyCom.SetGold(true, startValues.AMOUNT_GOLD_MASTER);

            EconomyEnt_EconomyCom.SetFood(false, startValues.AMOUNT_FOOD_OTHER);
            EconomyEnt_EconomyCom.SetWood(false, startValues.AMOUNT_WOOD_OTHER);
            EconomyEnt_EconomyCom.SetOre(false, startValues.AMOUNT_ORE_OTHER);
            EconomyEnt_EconomyCom.SetIron(false, startValues.AMOUNT_IRON_OTHER);
            EconomyEnt_EconomyCom.SetGold(false, startValues.AMOUNT_GOLD_OTHER);
    ...
    {
        var cellGO = Instance.ResourcesLoad.PrefabConfig.CellGO;
        var whiteCellSR = Instance.ResourcesLoad.SpritesConfig.WhiteSprite;
        var blackCellSR = Instance.ResourcesLoad.SpritesConfig.BlackSprite;

        var CellsGO = new GameObject[startValues.CELL_COUNT_X, startValues.CELL_COUNT_Y];
        ...
        for (int x = 0; x < startValues.CELL_COUNT_X; x++)
        {
            for (int y = 0; y < startValues.CELL_COUNT_Y; y++)
            {
                if (y % 2 == 0)
                {
                    if (x % 2 == 0)
                    {
                        CellsGO[x, y] = CreateGameObject(cellGO, blackCellSR, x, y, Main.Instance.gameObject);
                        SetActive(CellsGO[x, y], x, y);
                    }
                    if (x % 2 != 0)
                    {
                        CellsGO[x, y] = CreateGameObject(cellGO, whiteCellSR, x, y, Main.Instance.gameObject);
                        SetActive(CellsGO[x, y], x, y);
                    }
                }
                if (y % 2 != 0)
                ...
                CellsGO[x, y].transform.rotation = Main.Instance.IsMasterClient ? new Quaternion(0, 0, 0, 0) : new Quaternion(0, 0, 180, 0);

                var isStartedDict = new Dictionary<bool, bool>();
                if (Main.Instance.TestType == TestTypes.Standart)
                {
            ...
            for (int x = 0; x < Xamount; x++)
            {
                for (int y = 0; y < Yamount; y++)
                {
                    ...
                    if (!CellEnvEnt_CellEnvCom(x, y).HaveMountain)
                    {
                        ...
                        random = UnityEngine.Random.Range(1, 100);
                        if (random <= startValues.PERCENT_HILL)
                        ...
                        {
                            random = UnityEngine.Random.Range(1, 100);
                            if (random <= startValues.PERCENT_FOOD)
