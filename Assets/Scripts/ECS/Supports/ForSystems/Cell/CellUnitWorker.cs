...
internal sealed class CellUnitWorker : CellWorker
{
    ...
    internal CellUnitWorker(ECSManager eCSmanager, CellBaseOperations cellBaseOperations, ResourcesCommComponent resourcesCommComponent) : base(eCSmanager)
    {
        _cellBaseOperations = cellBaseOperations;
        _startValuesGameConfig = resourcesCommComponent.StartValuesGameConfig;
    }
    ...
    internal int MaxAmountHealth(params int[] xy)
    {
        switch (_eGM.CellUnitEnt_UnitTypeCom(xy).UnitType)
        {
            ...
            case UnitTypes.Pawn:
                return _startValuesGameConfig.AMOUNT_HEALTH_PAWN + _eGM.UnitInventorEnt_UpgradeUnitCom.AmountUpgrades(UnitTypes.Pawn, _eGM.CellUnitEnt_CellOwnerCom(xy).IsMasterClient) * _startValuesGameConfig.HEALTH_UPGRADE_ADDING_PAWN;

            case UnitTypes.Rook:
                return _startValuesGameConfig.AMOUNT_HEALTH_ROOK + _eGM.UnitInventorEnt_UpgradeUnitCom.AmountUpgrades(UnitTypes.Rook, _eGM.CellUnitEnt_CellOwnerCom(xy).IsMasterClient) * _startValuesGameConfig.HEALTH_UPGRADE_ADDING_ROOK;

            case UnitTypes.Bishop:
                return _startValuesGameConfig.AMOUNT_HEALTH_BISHOP + _eGM.UnitInventorEnt_UpgradeUnitCom.AmountUpgrades(UnitTypes.Bishop, _eGM.CellUnitEnt_CellOwnerCom(xy).IsMasterClient) * _startValuesGameConfig.HEALTH_UPGRADE_ADDING_BISHOP;
    ...
    internal int SimplePowerDamage(params int[] xy)
    {
        ...
        switch (_eGM.CellUnitEnt_UnitTypeCom(xy).UnitType)
        {
            ...
            case UnitTypes.Pawn:
                return _startValuesGameConfig.SIMPLE_POWER_DAMAGE_PAWN + _eGM.UnitInventorEnt_UpgradeUnitCom.AmountUpgrades(UnitTypes.Pawn, _eGM.CellUnitEnt_CellOwnerCom(xy).IsMasterClient) * _startValuesGameConfig.DAMAGE_UPGRADE_ADDING_PAWN;

            case UnitTypes.Rook:
                return _startValuesGameConfig.SIMPLE_POWER_DAMAGE_ROOK + _eGM.UnitInventorEnt_UpgradeUnitCom.AmountUpgrades(UnitTypes.Rook, _eGM.CellUnitEnt_CellOwnerCom(xy).IsMasterClient) * _startValuesGameConfig.DAMAGE_UPGRADE_ADDING_ROOK;

            case UnitTypes.Bishop:
                return _startValuesGameConfig.SIMPLE_POWER_DAMAGE_BISHOP + _eGM.UnitInventorEnt_UpgradeUnitCom.AmountUpgrades(UnitTypes.Bishop, _eGM.CellUnitEnt_CellOwnerCom(xy).IsMasterClient) * _startValuesGameConfig.DAMAGE_UPGRADE_ADDING_BISHOP;
    ...
    internal int UniquePowerDamage(params int[] xy)
    {
        ...
        switch (_eGM.CellUnitEnt_UnitTypeCom(xy).UnitType)
        {
            ...
            case UnitTypes.Pawn:
                return (int)(SimplePowerDamage(xy) * _startValuesGameConfig.RATION_UNIQUE_POWER_DAMAGE_PAWN);

            case UnitTypes.Rook:
                return (int)(SimplePowerDamage(xy) * _startValuesGameConfig.RATION_UNIQUE_POWER_DAMAGE_ROOK);

            case UnitTypes.Bishop:
                return (int)(SimplePowerDamage(xy) * _startValuesGameConfig.RATION_UNIQUE_POWER_DAMAGE_BISHOP);
    ...
    internal int PowerProtection(params int[] xy)
    {
        ...
        if (_eGM.CellUnitEnt_CellUnitCom(xy).IsProtected)
        {
            ...
            {
                case UnitTypes.King:
                    powerProtection += (int)(SimplePowerDamage(xy) * _startValuesGameConfig.PERCENT_FOR_PROTECTION_KING);
                    break;

                case UnitTypes.Pawn:
                    powerProtection += (int)(SimplePowerDamage(xy) * _startValuesGameConfig.PERCENT_FOR_PROTECTION_PAWN);
                    break;

                case UnitTypes.Rook:
                    powerProtection += (int)(SimplePowerDamage(xy) * _startValuesGameConfig.PERCENT_FOR_PROTECTION_ROOK);
                    break;

                case UnitTypes.Bishop:
                    powerProtection += (int)(SimplePowerDamage(xy) * _startValuesGameConfig.PERCENT_FOR_PROTECTION_BISHOP);
                    break;
            }
        }
    ...
    internal bool HaveMaxSteps(params int[] xy)
    {
        ...
        {
            case UnitTypes.King:
                return _eGM.CellUnitEnt_CellUnitCom(xy).AmountSteps == _startValuesGameConfig.STANDART_AMOUNT_STEPS_KING;

            case UnitTypes.Pawn:
                return _eGM.CellUnitEnt_CellUnitCom(xy).AmountSteps == _startValuesGameConfig.STANDART_AMOUNT_STEPS_PAWN;

            case UnitTypes.Rook:
                return _eGM.CellUnitEnt_CellUnitCom(xy).AmountSteps == _startValuesGameConfig.STANDART_AMOUNT_STEPS_ROOK;

            case UnitTypes.Bishop:
                return _eGM.CellUnitEnt_CellUnitCom(xy).AmountSteps == _startValuesGameConfig.STANDART_AMOUNT_STEPS_BISHOP;
        }
        return false;
    ...
    internal void RefreshAmountSteps(params int[] xy)
    {
        ...
        {
            case UnitTypes.King:
                _eGM.CellUnitEnt_CellUnitCom(xy).AmountSteps = _startValuesGameConfig.STANDART_AMOUNT_STEPS_KING;
                break;

            case UnitTypes.Pawn:
                _eGM.CellUnitEnt_CellUnitCom(xy).AmountSteps = _startValuesGameConfig.STANDART_AMOUNT_STEPS_PAWN;
                break;

            case UnitTypes.Rook:
                _eGM.CellUnitEnt_CellUnitCom(xy).AmountSteps = _startValuesGameConfig.STANDART_AMOUNT_STEPS_ROOK;
                break;

            case UnitTypes.Bishop:
                _eGM.CellUnitEnt_CellUnitCom(xy).AmountSteps = _startValuesGameConfig.STANDART_AMOUNT_STEPS_BISHOP;
                break;
