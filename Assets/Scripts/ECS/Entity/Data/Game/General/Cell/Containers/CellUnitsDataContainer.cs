...
{
    public struct CellUnitsDataContainer
    {
        private static EcsEntity[,] _cellUnitEnts;

        internal CellUnitsDataContainer(EcsWorld gameWorld)
        {
        ...
        internal static int PowerProtection(int[] xy)
        {
            ...
            switch (CellBuildDataContainer.GetBuildingType(xy))
            {
        ...
        internal static List<int[]> GetCellsForShift(int[] xy)
        {
            ...
            {
                if (!CellEnvirDataContainer.HaveEnvironment(EnvironmentTypes.Mountain, xy1) && !HaveAnyUnit(xy1))
                {
                    if (AmountSteps(xy) >= CellEnvirDataContainer.NeedAmountSteps(xy1) || HaveMaxAmountSteps(xy))
                    {
                        list.Add(xy1);
                    }
                }
        ...
        internal static void GetCellsForAttack(Player playerFrom, out List<int[]> availableCellsSimpleAttack, out List<int[]> availableCellsUniqueAttack, int[] xy)
        {
            ...
            if (IsMelee(xy))
            {
                for (DirectTypes directType1 = default; directType1 <= DirectTypes.LeftDown; directType1++)
                {
                    ...
                    if (!CellEnvirDataContainer.HaveEnvironment(EnvironmentTypes.Mountain, xy1))
                    {
                        if (CellEnvirDataContainer.NeedAmountSteps(xy1) <= AmountSteps(xy) || HaveMaxAmountSteps(xy))
                        {
                            if (HaveAnyUnit(xy1))
                            {
                                if (HaveOwner(xy1))
            ...
            else
            {
                for (DirectTypes directType1 = default; directType1 <= DirectTypes.LeftDown; directType1++)
                {
                    ...
                    if (CellViewContainer.IsActiveSelfParentCell(xy1))
                    {
                        ...
                        if (CellUnitsDataContainer.IsVisibleUnit(PhotonNetwork.IsMasterClient, xy2))
                        {
                            if (UnitType(xy) == UnitTypes.Rook || UnitType(xy) == UnitTypes.RookCrossbow)
                            {
                                if (directType1 == DirectTypes.Left || directType1 == DirectTypes.Right || directType1 == DirectTypes.Down || directType1 == DirectTypes.Up)
                                {
                                    if (!CellEnvirDataContainer.HaveEnvironment(EnvironmentTypes.Mountain, xy2))
                                    {
                                        if (HaveAnyUnit(xy2))
                                        {
                                            if (HaveOwner(xy2))
        ...
        internal static List<int[]> GetStartCellsForSettingUnit(Player player)
        {
            ...
            for (int x = 0; x < CellValues.CELL_COUNT_X; x++)
                for (int y = 0; y < CellValues.CELL_COUNT_Y; y++)
                {
                    ...
                    if (!CellEnvirDataContainer.HaveEnvironment(EnvironmentTypes.Mountain, xy))
                    {
                        if (!HaveAnyUnit(xy))
                        {
